...
class Heap {
  ...
  _up(k) {
    const { arr, _parent, _swap, compare } = this;
    while(k > 1 && compare(arr[k], arr[_parent(k)])) {
      _swap(k, _parent(k));
      k = _parent(k);
    }
  ...
  _down(k) {
    const size = this.size();
    const { compare, _left, _right, _swap } = this;
    while(_left(k) <= size) {
      let child = this._left(k);
      if (_right(k) <= size && compare(arr[child], arr[_right(k)])) {
        child = _right(k);
      }
      if (!compare(arr[child], arr[k])) return;
      _swap(k, child);
      k = child;
    }
  }
